...
export class ChequesInbank implements OnInit {
  ...
  constructor() {
    // DatePicker configs
    const base = { containerClass: 'theme-dark-blue', dateInputFormat: 'DD-MMM-YYYY', showWeekNumbers: false, maxDate: new Date() };
    Object.assign(this.ptransactiondateConfig, base);
    Object.assign(this.pchequecleardateConfig, base);
    Object.assign(this.brsfromConfig, base);
    Object.assign(this.brstoConfig, base);
  ...
  getCurrentPageTotal(dt: any): number {
    ...
    const pageData = (dt._value as any[]).slice(first, first + rows);
          ...
          next: (res: any) => {
            if (res) {
              ...
              if (bankid == 0) {
                ...
                const total = (this.BanksList ?? []).reduce((s: number, b: any) => s + (b.pbankbalance ?? 0), 0);
  ...
  SelectBank(event: any) {
    if (!event) {
      ...
      this.bankbalance = 0; this.bankbalancetype = '';
      const d = new Date();
      this.brsdate = d.getDate().toString().padStart(2, '0') + '-' +
        d.toLocaleString('en-US', { month: 'short' }) + '-' + d.getFullYear();
    } else {
      this.banknameshowhide = true; this.bankdetails = event;
      this.bankid = event.pbankid; this.bankname = event.pbankname;
  ...
  private _recalculateCountsForBank(): void {
    if (this.bankid == 0) { this.CountOfRecords(); return; }
    const onHand = this.ChequesInBankData.filter((d: any) => d?.pdepositbankid == this.bankid);
    const cr = this.ChequesClearReturnData.filter((d: any) => d?.pdepositbankid == this.bankid);
  ...
  ChequesDeposited() {
    ...
    this.modeofreceipt = 'CHEQUE'; this.status = 'chequesdeposited'; this.pdfstatus = 'Cheques Deposited';
  ...
  ChequesDeposited1() {
    ...
    this.gridData = JSON.parse(JSON.stringify(grid)); this.gridDatatemp = [...this.gridData];
    this.showicons = this.gridData.length > 0;
    this.amounttotal = this.gridData.reduce((s: number, c: any) => s + (c.ptotalreceivedamount || 0), 0);
    this.page.totalElements = this.gridData.length; this.page.totalPages = Math.ceil(this.gridData.length / (this.page.size || 10));
  }

  OnlineReceipts() {
    this.chequeboxshoworhide = true; this.amounttotal = 0; this.fromdate = ''; this.todate = '';
    this.pageSetUp(); this.gridData = []; this.gridDatatemp = [];
    this.fromFormName === 'fromChequesStatusInformationForm' ? this.GridColumnsHide() : this.GridColumnsShow();
    this.status = 'onlinereceipts'; this.pdfstatus = 'Online Receipts'; this.modeofreceipt = 'ONLINE';
    this.GetChequesInBankforSearchDeposit(this.bankid, this.startindex, this.endindex, this._searchText);
  }
  ...
  Cleared() {
    this.chequeboxshoworhide = false; this.amounttotal = 0; this.fromdate = ''; this.todate = '';
    this.datetitle = 'Cleared Date'; this.gridData = []; this.gridDatatemp = [];
    this.GridColumnsHide(); this.brsdateshowhidecleared = true; this.brsdateshowhidereturned = false;
  ...
  Returned() {
    ...
    this.GridColumnsHide(); this.brsdateshowhidecleared = false; this.brsdateshowhidereturned = true;
  ...
  GetChequesInBankforSearchDeposit(bankid: any, startindex: any, endindex: any, searchText: any) {
    ...
    forkJoin([data$, count$]).subscribe({
      next: (data: any) => {
        ...
        this.page.totalElements = this.bankid == 0
          ? (this._countData?.total_count || 0)
  ...
  onBrsFromDateChange(event: Date): void {
    if (event) this.brstoConfig = { ...this.brstoConfig, minDate: event };
  }

  onBrsToDateChange(event: Date): void {
    if (event) this.brsfromConfig = { ...this.brsfromConfig, maxDate: event };
  }

  ShowBrsClear() {
    this._searchText = ''; this.gridData = []; this.cleared = 0;
  ...
  ShowBrsReturn() {
    this._searchText = ''; this.gridData = []; this.returned = 0;
  ...
  showSearchText(_event: any) {
    ...
    if (this.fromFormName === 'fromChequesStatusInformationForm') {
      ...
      if (searchText) {
        ...
        else if (this.status === 'chequesdeposited') this.gridData = JSON.parse(JSON.stringify(oH.filter((d: any) => d.ptypeofpayment === 'CHEQUE')));
        else if (this.status === 'onlinereceipts') this.gridData = JSON.parse(JSON.stringify(oH.filter((d: any) => d.ptypeofpayment !== 'CHEQUE')));
        else if (this.status === 'cleared') this.gridData = JSON.parse(JSON.stringify(cr.filter((d: any) => d.pchequestatus === 'Y')));
        else if (this.status === 'returned') this.gridData = JSON.parse(JSON.stringify(cr.filter((d: any) => d.pchequestatus === 'R')));
        this.gridDatatemp = [...this.gridData];
        this.amounttotal = this.gridData.reduce((s: number, c: any) => s + (c.ptotalreceivedamount || 0), 0);
      } else {
  ...
  onRowClearCheck(event: any, row: any) {
    if (event.target.checked) {
      ...
      row.pdepositstatus = false; row.pchequestatus = 'N'; row.preturnstatus = false; row.preferencetext = '';
    }
    this.selectedamt = this.gridData.filter((r: any) => r.pdepositstatus).reduce((s: number, r: any) => s + (r.ptotalreceivedamount || 0), 0);
  ...
  CheckedReturn(event: any, data: any) {
    ...
    if (event.target.checked) {
      ...
      if (!receiptdate || (chequecleardate && new Date(chequecleardate).getTime() >= new Date(receiptdate).getTime())) {
        data.preturnstatus = true; data.pdepositstatus = false; data.pchequestatus = 'R';
        this.returnChargesError = false; this.chequenumber = data.pChequenumber;
        ...
        setTimeout(() => {
          ...
          if (el) el.value = String(this.minimumReturnCharge || 250);
        }, 50);
      } else {
        data.preturnstatus = false; data.pchequestatus = 'N'; event.target.checked = false;
    ...
    for (let i = 0; i < this.gridData.length; i++) {
      if (this.gridData[i]?.preceiptid == data.preceiptid) { this.gridData[i] = data; break; }
    }
  }
  ...
  CheckedClear(event: any, data: any) {
      ...
      if (receiptdate && chequecleardate && new Date(chequecleardate).getTime() < new Date(receiptdate).getTime()) {
        ...
        if (parseInt(this.roleid, 10) !== 2) {
          ...
          this.gridData.forEach(el => {
            if (el?.pChequenumber == data.pChequenumber && data.cheque_bank == el.cheque_bank && data.receipt_branch_name == el.receipt_branch_name) {
              el.pdepositstatus = true; el.preturnstatus = false; el.pchequestatus = 'Y';
            }
          });
        } else { data.pdepositstatus = true; data.preturnstatus = false; data.pchequestatus = 'Y'; }
      }
    } else {
    ...
    this.selectedamt = 0;
    this.gridData.forEach((el: any) => { if (el?.pdepositstatus) this.selectedamt += el?.ptotalreceivedamount || 0; });
  }
  ...
  CancelChargesOk(_event: any) {
    ...
    if (!rawValue || value < minimum) {
      ...
      this._commonService.showWarningMessage('Minimum Amount Should Be ' + minimum); return;
    }
    this.returnChargesError = false;
    for (let i = 0; i < this.gridData.length; i++) {
      if (this.gridData[i].preceiptid == this.PopupData.preceiptid) { this.gridData[i].pactualcancelcharges = value; }
    }
    const modal = document.getElementById('add-detail');
  ...
  closeReturnModal() {
    if (this.PopupData) {
      this.PopupData.preturnstatus = false; this.PopupData.pchequestatus = 'N';
      for (let i = 0; i < this.gridData.length; i++) {
        if (this.gridData[i].preceiptid == this.PopupData.preceiptid) {
          this.gridData[i].preturnstatus = false; this.gridData[i].pchequestatus = 'N'; break;
        }
      }
    }
    this.returnChargesError = false;
  ...
  validateSave(): boolean {
    ...
    if (this.DataForSaving.length > 0) { isvalid = true; }
      ...
      const selectrecords = this.gridData.filter(el => el.pchequestatus === 'Y' || el.pchequestatus === 'R');
      if (!this.showhidegridcolumns) {
        ...
        else if (!selectrecords.length) { this._commonService.showWarningMessage('Please Select records'); isvalid = false; }
      }
    }
  ...
  private _prepareSaveItems(items: any[], isAuto: boolean) {
    for (const item of items) {
      item.pCreatedby = '1';
      item.pdepositeddate = this._commonService.getFormatDateNormal(this._commonService.getDateObjectFromDataBase(item.pdepositeddate));
      item.preceiptdate = this._commonService.getFormatDateNormal(this._commonService.getDateObjectFromDataBase(item.preceiptdate));
      item.pchequedate = this._commonService.getFormatDateNormal(this._commonService.getDateObjectFromDataBase(item.pchequedate));
      item.pipaddress = this._commonService.getIpAddress();
      item.pactualcancelcharges = item.pactualcancelcharges ?? 0;
      if (isAuto) { item.pchequestatus = 'Y'; item.preferencetext = item.preferencetext + '-' + new Date().getFullYear(); }
      else { item.preferencetext = item.preferencetext + '-' + new Date().getFullYear(); }
    }
  }
  ...
  Clear() {
    ...
    this.ChequesInBankForm.reset({ ptransactiondate: today, pchequecleardate: today, bankname: null, pfrombrsdate: yesterday, ptobrsdate: today, schemaname: this._commonService.getschemaname(), searchtext: '', receipttype: 'Adjusted', auto_brs_type: 'Upload' });
    this.BrsDateForm.reset({ frombrsdate: yesterday, tobrsdate: today });
    this.bankid = 0; this.bankname = ''; this.bankbalance = 0; this.bankbalancetype = ''; this.brsdate = ''; this.banknameshowhide = false;
    this.gridData = []; this.gridDatatemp = []; this.ChequesInBankData = []; this.ChequesClearReturnData = [];
    this.modeofreceipt = 'ALL'; this.status = 'onlinereceipts'; this.selectedTab = 'onlinereceipts';
    this._searchText = ''; this.fromdate = ''; this.todate = ''; this.preferdrows = false;
    this.ChequesInBankValidation = {}; this.amounttotal = 0; this.selectedamt = 0;
  ...
  checkDuplicateValueslatest(event: any, rowIndex: any, row: any) {
    ...
    this.gridData.forEach(el => {
      if (el.pChequenumber === row.pChequenumber && el.cheque_bank === row.cheque_bank && el.receipt_branch_name === row.receipt_branch_name) {
        if (el.pdepositstatus === true) el.preferencetext = value;
        return;
      }
      if ((el.pdepositstatus === true || el.preturnstatus === true) && el.preferencetext?.toString().trim() !== '' && el.preferencetext?.toString().toLowerCase() === value.toLowerCase()) isDuplicate = true;
    });
    if (isDuplicate) {
      this._commonService.showWarningMessage('Already Exist'); row.preferencetext = '';
  ...
  validateDuplicates() {
    const arr = this.gridData.filter(el => el.pchequestatus === 'Y' || el.pchequestatus === 'R');
    let count = 0;
    for (let i = 0; i < arr.length; i++)
      for (let k = 0; k < arr.length; k++)
        if (arr[i].pChequenumber !== arr[k].pChequenumber &&
          !this._commonService.isNullOrEmptyString(arr[i].preferencetext) && !this._commonService.isNullOrEmptyString(arr[k].preferencetext) &&
          arr[i].preferencetext === arr[k].preferencetext) count++;
    return count;
  }

  emptyValuesFound() {
    return this.gridData.filter(el => el.pdepositstatus || el.preturnstatus).some(item => item.preferencetext === '');
  }

  checkValidations(group: FormGroup, isValid: boolean): boolean {
    try { Object.keys(group.controls).forEach(key => isValid = this.GetValidationByControl(group, key, isValid)); }
        ...
        if (ctrl instanceof FormGroup) this.checkValidations(ctrl, isValid);
          ...
          if (ctrl.errors || ctrl.invalid || ctrl.touched || ctrl.dirty) {
            ...
            for (const errkey in ctrl.errors) {
              if (errkey) {
                this.ChequesInBankValidation[key] += this._commonService.getValidationMessage(ctrl, errkey, label, key, '') + ' ';
                isValid = false;
              }
            }
          }
  ...
  BlurEventAllControll(fromgroup: FormGroup): any {
    try { Object.keys(fromgroup.controls).forEach(key => this.setBlurEvent(fromgroup, key)); }
    catch (e) { this.showErrorMessage('e'); return false; }
  }
  ...
  pdfOrprint(printorpdf: any) {
    ...
    const headers = ['Cheque/\nReference No.', 'Branch Name', 'Amount', 'Receipt ID', 'Receipt Date', 'Deposited Date', ...(hasDateCol ? [isCleared ? 'Cleared Date' : 'Returned Date'] : []), 'Transaction Mode', 'Cheque Bank Name', 'Cheque Branch Name', 'Party'];
    const colStyles: Record<number, any> = { 0: { cellWidth: 30, halign: 'center' }, 1: { cellWidth: 35, halign: 'left' }, 2: { cellWidth: 22, halign: 'right' }, 3: { cellWidth: 18, halign: 'center' }, 4: { cellWidth: 20, halign: 'center' }, 5: { cellWidth: 20, halign: 'center' } };
    if (hasDateCol) { colStyles[6] = { cellWidth: 20, halign: 'center' }; colStyles[7] = { cellWidth: 22, halign: 'center' }; colStyles[8] = { cellWidth: 28, halign: 'left' }; colStyles[9] = { cellWidth: 24, halign: 'left' }; colStyles[10] = { cellWidth: 38, halign: 'left' }; }
    else { colStyles[6] = { cellWidth: 24, halign: 'center' }; colStyles[7] = { cellWidth: 32, halign: 'left' }; colStyles[8] = { cellWidth: 28, halign: 'left' }; colStyles[9] = { cellWidth: 46, halign: 'left' }; }
    const data: any[][] = [];
    this.gridData.forEach((e: any) => {
      const amt = Number(e?.ptotalreceivedamount || 0); this.Totlaamount += amt;
      data.push([e?.pChequenumber || '', e?.pbranchname || '', this._commonService.convertAmountToPdfFormat(amt), e?.preceiptid || '',
    ...
    data.push(totalRow);
    this._commonService._downloadchequesReportsPdf('Cheques In Bank', data, headers, colStyles, 'landscape', this.bankname || '', this.brsdate || '', this.pdfstatus || '', printorpdf, ' ');
  }
  ...
  export(): void {
    ...
    const buildRows = (gridData: any[]): any[] => gridData.map((e: any) => {
      const row: Record<string, any> = {
        'Cheque/ Reference No.': e?.pChequenumber || '', 'Branch Name': e?.pbranchname || '',
    ...
    if (isCleared || isReturned) { this._commonService.exportAsExcelFile(buildRows(this.gridData), 'Cheques in Bank'); return; }
    this._accountingtransaction.GetChequesInBankData(
      this.bankid, this._commonService.getschemaname(), this._commonService.getbranchname(),
  ...
  pdfContentData() {
    ...
    this.previewdetails.forEach((obj: any, idx: number) => {
      ...
      const ExportRightSideData = [obj.psubscribername?.trim() + ', ', (obj.paddress ?? '') + '.'];
  ...
  chequereturnvoucherpdf() {
    ...
    this.chequerwturnvoucherdetails.forEach((obj: any, idx: number) => {
      ...
      doc.text(address.substr(0, 115), 110, 15, { align: 'center' });
      doc.text('' + address.substring(115) + '', 110, 18);
      if (!isNullOrEmptyString(Companyreportdetails.pCinNo)) doc.text('CIN : ' + Companyreportdetails.pCinNo + '', 90, 22);
  ...
  AutoBrs() {
    if (this.ChequesInBankForm.controls['bankname'].value) {
      ...
      this.GetChequesInBank_load(this.bankid);
    } else { this._commonService.showWarningMessage('Please Select Bank'); this.gridData = []; }
  }
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      this.PreDefinedAutoBrsArrayData = this.Exceldata.map((row: any) => ({
        ...
        chqueno: row[1], chequeamount: row[2], preferencetext: row[3], preceiptype: row[4], uploadtype: row[5]
      }));
      ...
      if (new Date(chequecleardate).getTime() >= new Date().getTime()) {
        ...
        this._accountingtransaction.GetChequesInBankData(
          ...
          for (const d of this.autoBrsDuplicates)
            if (d.pChequenumber == row.pChequenumber && d.pchequedate == row.pchequedate) this.autoBrsData.push(d);
          this.selectedamt = this.autoBrsData.reduce((s: number, c: any) => s + (c.ptotalreceivedamount || 0), 0);
          this.gridLoading.set(false);
        });
      } else {
        this._commonService.showWarningMessage('Cheque Clear Date Should be Greater than or Equal to Deposited Date');
