...
//     let finalFromDate: Date = new Date();
//     let finalToDate: Date = new Date();
...
//     setTimeout(() => {
//       this.ChequesOnHandForm.patchValue({ pfrombrsdate: new Date(), ptobrsdate: new Date() });
//       this.ChequesOnHandForm.controls['pfrombrsdate'].markAsDirty();
//       this.ChequesOnHandForm.controls['ptobrsdate'].markAsDirty();
...
//     setTimeout(() => {
//       this.BrsDateForm.patchValue({ frombrsdate: new Date(), tobrsdate: new Date() });
...
//     if (isCancelled) {
//       this._fetchDataAndBuildPdf(isCancelled ? 'CANCELLED' : null, 'C', isDeposited, isCancelled, printOrPdf);
//       return;
//     }
//     if (isDeposited) {
//       this._fetchDataAndBuildPdf('DEPOSITED', 'P', isDeposited, isCancelled, printOrPdf);
//       return;
//     }
...
//         0, totalCount, '', apiMode, 'PDF',
//         this._commonService.getCompanyCode(), this._commonService.getBranchCode()
//       )
//       .subscribe((result: any) => {
//         let gridData: any[] = result?.pchequesOnHandlist || [];
//         if (this.status() === 'chequesreceived') gridData = gridData.filter((x: any) => x.ptypeofpayment === 'CHEQUE');
//         else if (this.status() === 'onlinereceipts') gridData = gridData.filter((x: any) => x.ptypeofpayment !== 'CHEQUE');
//         this._buildPdfRows(gridData.length ? gridData : [...this.gridData()], printOrPdf, isDeposited, isCancelled);
//       });
//   }
...
//       this._accountingtransaction
//         .DataFromBrsDatesChequesOnHand(this.fromdate, this.todate, this.bankid, mode, this._searchText || '0', 0, 99999)
//         .subscribe({
//           next: (result: any) => {
...
//     } else {
//       this._accountingtransaction
//         .GetChequesOnHandData(this.bankid, this._commonService.getschemaname(), this._commonService.getbranchname(), 0, 99999, '', 'ALL', 'PDF', this._commonService.getCompanyCode(), this._commonService.getBranchCode())
//         .subscribe({
//           next: (result: any) => {
//             let data: any[] = (result?.pchequesclearreturnlist || []).filter((x: any) => x.pchequestatus === statusFilter);
//             if (isDeposited && this.bankid && this.bankid !== 0) data = data.filter((x: any) => x.pdepositbankid === this.bankid);
...
//       ? ['Cheque/Reference No.', 'Branch Name', 'Amount', 'Receipt No', 'Receipt Date', 'Cheque Date', this.datetitle, 'Transaction Mode', 'Cheque Bank Name', 'Party']
//       : ['Cheque/Reference No.', 'Branch Name', 'Amount', 'Receipt No', 'Receipt Date', 'Cheque Date', 'Transaction Mode', 'Cheque Bank Name', 'Party', 'Self Cheque', 'Chit Status', 'Deposited Bank'];

//     const colWidthHeight = isDepositOrCancel
//       ? { 0: { cellWidth: 33, halign: 'center' }, 1: { cellWidth: 26, halign: 'left' }, 2: { cellWidth: 24, halign: 'right' }, 3: { cellWidth: 20, halign: 'center' }, 4: { cellWidth: 25, halign: 'center' }, 5: { cellWidth: 25, halign: 'center' }, 6: { cellWidth: 23, halign: 'center' }, 7: { cellWidth: 25, halign: 'center' }, 8: { cellWidth: 40, halign: 'center' }, 9: { cellWidth: 30, halign: 'left' } }
//       : { 0: { cellWidth: 22, halign: 'center' }, 1: { cellWidth: 26, halign: 'left' }, 2: { cellWidth: 24, halign: 'right' }, 3: { cellWidth: 20, halign: 'center' }, 4: { cellWidth: 20, halign: 'center' }, 5: { cellWidth: 20, halign: 'center' }, 6: { cellWidth: 20, halign: 'center' }, 7: { cellWidth: 25, halign: 'center' }, 8: { cellWidth: 25, halign: 'left' }, 9: { cellWidth: 35, halign: 'left' }, 10: { cellWidth: 15, halign: 'center' }, 11: { cellWidth: 20, halign: 'center' }, 12: { cellWidth: 20, halign: 'left' } };
...
//       const branchName = typeof element.pbranchname === 'string' && element.pbranchname.trim() !== '' ? element.pbranchname : '';
//       const selfCheque = element.selfchequestatus === true ? 'Yes' : element.selfchequestatus === false ? 'No' : '';

//       let totalreceivedamt = '';
//       try {
//         if (element.ptotalreceivedamount != null && element.ptotalreceivedamount !== 0) {
//           const amt = parseFloat(element.ptotalreceivedamount) || 0;
//           totalreceivedamt = this._commonService.convertAmountToPdfFormat(this._commonService.currencyformat(parseFloat(amt.toFixed(2))));
...
//       rows.push(isDepositOrCancel
//         ? [element.pChequenumber || '', branchName, totalreceivedamt, element.receiptnumbers || '', datereceipt, chequedate, depositeddate, element.ptypeofpayment || '', element.cheque_bank || '', element.ppartyname || '']
//         : [element.pChequenumber || '', branchName, totalreceivedamt, element.receiptnumbers || '', datereceipt, chequedate, element.ptypeofpayment || '', element.cheque_bank || '', element.ppartyname || '', selfCheque, element.chitstatus || '', element.pdepositedBankName || '']
//       );
//     });
...
//     const totalRow: any[] = [
//       { content: 'Total', colSpan: 2, styles: { halign: 'right', fontSize: 16, fontStyle: 'bold' } },
//       { content: amounttotal, styles: { halign: 'right', fontSize: 16, fontStyle: 'bold' } },
//       ...Array(totalColCount - 2).fill('')
//     ];
//     rows.push(totalRow);
...
//     const hasSelectedRows = this.gridData().some(row => row.pdepositstatus === true || row.pcancelstatus === true);
//     if (!hasSelectedRows) {
...
//     const control = this.ChequesOnHandForm.get('bankname') as any;
...
//           this.DataForSaving.push({
//             ...this._mapRowToSavePayload(row, status)
//           });
//         }
//       });
...
//           ptransactiondate: this.datepipe.transform(this.ChequesOnHandForm.controls['ptransactiondate'].value, 'yyyy-MM-dd'),
//           pchequecleardate: '',
//           pcaobranchcode: '',
//           pcaobranchname: '',
//           pcaobranchid: '',
...
//           pchequesOnHandlist: this.DataForSaving,
...
//               this.gridData.update(rows => rows.map(row => ({ ...row, pdepositstatus: false, pcancelstatus: false, pchequestatus: 'N' })));
//               this.DataForSaving = [];
//               this.count = 0;
//               this.checked = false;
...
//               this.GetChequesOnHand_Load(this.bankid);
//             } else {
//               this._commonService.showWarningMessage(data?.message || 'Save failed');
//               this.checked = false;
//               this.disablesavebutton.set(false);
//               this.buttonname.set('Save');
//             }
...
//     return {
//       pRecordid: str(row.pRecordid), pUpiname: str(row.pUpiname), pUpiid: str(row.pUpiid),
//       pBankconfigurationId: str(row.pBankconfigurationId), pBankName: str(row.pBankName),
//       ptranstype: str(row.ptranstype), ptypeofpayment: str(row.ptypeofpayment),
//       pChequenumber: str(row.pChequenumber), pchequedate: str(row.pchequedate),
//       pchequedepositdate: str(row.pchequedepositdate), pchequecleardate: str(row.pchequecleardate),
//       pbankid: str(row.pbankid), branchid: str(row.branchid), pCardNumber: str(row.pCardNumber),
//       pdepositbankid: status === 'P' ? (this.bankdetails ? str(this.bankdetails.pbankid) : '0') : str(row.pdepositbankid),
//       pdepositbankname: str(row.pdepositbankname), pAccountnumber: str(row.pAccountnumber),
//       challanaNo: str(row.challanaNo), preceiptid: str(row.preceiptid), preceiptdate: str(row.preceiptdate),
//       pmodofreceipt: str(row.pmodofreceipt), ptotalreceivedamount: str(row.ptotalreceivedamount),
//       pnarration: str(row.pnarration), ppartyname: str(row.ppartyname), ppartyid: str(row.ppartyid),
//       pistdsapplicable: str(row.pistdsapplicable), pTdsSection: str(row.pTdsSection),
//       pTdsPercentage: str(row.pTdsPercentage), ptdsamount: str(row.ptdsamount),
//       ptdscalculationtype: str(row.ptdscalculationtype), ppartypannumber: str(row.ppartypannumber),
//       ppartyreftype: str(row.ppartyreftype), ppartyreferenceid: str(row.ppartyreferenceid),
//       preceiptslist: row.preceiptslist || [], pFilename: str(row.pFilename), pFilepath: str(row.pFilepath),
//       pFileformat: str(row.pFileformat), pCleardate: str(row.pCleardate),
//       pdepositeddate: status === 'P' ? '' : str(row.pdepositeddate),
//       ptdsaccountid: str(row.ptdsaccountid), preceiptrecordid: str(row.preceiptrecordid),
//       pTdsSectionId: str(row.pTdsSectionId), groupcode: str(row.groupcode), preceiptno: str(row.preceiptno),
//       formname: str(row.formname), chitpaymentid: str(row.chitpaymentid), adjustmentid: str(row.adjustmentid),
//       pdepositstatus: row.pdepositstatus === true ? 'true' : 'false',
//       pcancelstatus: row.pcancelstatus === true ? 'true' : 'false',
//       preturnstatus: str(row.preturnstatus), pbranchname: str(row.pbranchname), pchequestatus: status,
//       pcancelcharges: str(row.pcancelcharges), pactualcancelcharges: str(row.pactualcancelcharges) || '0',
//       pledger: str(row.pledger), cancelstatus: str(row.cancelstatus), returnstatus: str(row.returnstatus),
//       clearstatus: str(row.clearstatus), chqueno: str(row.chqueno), issueddate: str(row.issueddate),
//       chitgroupcode: str(row.chitgroupcode), chitgroupid: str(row.chitgroupid), ticketno: str(row.ticketno),
//       chequeamount: str(row.chequeamount), zpdaccountid: str(row.zpdaccountid), installmentno: str(row.installmentno),
//       schemesubscriberid: str(row.schemesubscriberid), contactid: str(row.contactid), schemetype: str(row.schemetype),
//       checksentryrecordid: str(row.checksentryrecordid), cheque_bank: str(row.cheque_bank),
//       selfchequestatus: str(row.selfchequestatus), branch_name: str(row.branch_name),
//       receipt_branch_name: str(row.receipt_branch_name), subscriber_details: str(row.subscriber_details),
//       chitReceiptNo: str(row.chitReceiptNo), total_count: str(row.total_count),
//       transactionNo: str(row.transactionNo), transactiondate: str(row.transactiondate),
//       chitstatus: str(row.chitstatus), chitgroupstatus: str(row.chitgroupstatus),
//       receiptnumbers: str(row.receiptnumbers), pdepositedBankid: str(row.pdepositedBankid),
//       pdepositedBankName: str(row.pdepositedBankName), preferencetext: str(row.preferencetext),
//       preceiptype: str(row.preceiptype), puploadeddate: str(row.puploadeddate),
//       subscriberbankaccountno: str(row.subscriberbankaccountno), pkgmsreceiptdate: str(row.pkgmsreceiptdate),
//       pCreatedby: this._commonService.getCreatedBy()?.toString() ?? '1'
//     };
//   }

//   Clear() {
//     const savedGridData = [...this.gridData()];
//     const savedGridDataTemp = [...this.gridDatatemp()];
//     const savedPage = { ...this.page() };
//     const savedTotalRecords = this.totalElements;
//     const savedAmountTotal = this.amounttotal();
...
//     this.gridData.set(savedGridData);
//     this.gridDatatemp.set(savedGridDataTemp);
//     this.page.set(savedPage);
//     this.totalElements = savedTotalRecords;
//     this.amounttotal.set(savedAmountTotal);
//     this.pageSetUp();
//   }
...
//       if (isDeposited) {
//         return { 'Cheque/ Reference No.': element.pChequenumber || '', 'Branch Name': branchName, 'Amount': totalreceivedamt, 'Receipt No': element.receiptnumbers || '', 'Receipt Date': datereceipt, 'Cheque Date': chequedate, 'Deposited Date': depositeddate, 'Transaction Mode': element.ptypeofpayment || '', 'Cheque Bank Name': element.cheque_bank || '', 'Party': element.ppartyname || '' };
//       } else if (isCancelled) {
//         return { 'Cheque/ Reference No.': element.pChequenumber || '', 'Branch Name': branchName, 'Amount': totalreceivedamt, 'Receipt No': element.receiptnumbers || '', 'Receipt Date': datereceipt, 'Cheque Date': chequedate, 'Cancelled Date': depositeddate, 'Transaction Mode': element.ptypeofpayment || '', 'Cheque Bank Name': element.cheque_bank || '', 'Party': element.ppartyname || '' };
//       } else {
//         return { 'Cheque/ Reference No.': element.pChequenumber || '', 'Branch Name': branchName, 'Amount': totalreceivedamt, 'Receipt No': element.receiptnumbers || '', 'Receipt Date': datereceipt, 'Cheque Date': chequedate, 'Transaction Mode': element.ptypeofpayment || '', 'Cheque Bank Name': element.cheque_bank || '', 'Party': element.ppartyname || '', 'Self Cheque': selfCheque, 'Chit Status': element.chitstatus || '', 'Deposited Bank': element.pdepositedBankName || '' };
//       }
//     });
...
export class ChequesOnhand implements OnInit {
  ...
  private _buildPdfRows(
    ...
    gridData.forEach((element: any) => {
      ...
      let totalreceivedamt = '';
      try {
        const rawAmt = element.ptotalreceivedamount;
        const amt = (rawAmt != null && rawAmt !== '' && !isNaN(Number(rawAmt)))
          ? parseFloat(Number(rawAmt).toFixed(2))
          : 0;

        if (amt !== 0) {
          const formatted = this._commonService.currencyformat(amt);
          totalreceivedamt = formatted != null
            ? this._commonService.convertAmountToPdfFormat(formatted)
            : amt.toFixed(2);
          this.Totalamount += amt;
        }
      } catch (e) {
        console.warn('Amount formatting error for element:', element, e);
        const fallbackAmt = parseFloat(element.ptotalreceivedamount) || 0;
        totalreceivedamt = fallbackAmt > 0 ? fallbackAmt.toFixed(2) : '';
        this.Totalamount += fallbackAmt;
      }
    ...
    let amounttotal = '';
    try {
      const formatted = this._commonService.currencyformat(this.Totalamount);
      amounttotal = formatted != null
        ? this._commonService.convertAmountToPdfFormat(formatted)
        : this.Totalamount.toFixed(2);
    } catch (e) {
      console.warn('Total amount formatting error:', e);
      amounttotal = this.Totalamount.toFixed(2);
    }
